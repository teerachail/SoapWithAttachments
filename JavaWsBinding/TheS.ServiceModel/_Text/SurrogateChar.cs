    ...
    struct SurrogateChar
    {
        ...
        public SurrogateChar(int ch)
        {
            if (ch < MinValue || ch > MaxValue)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(string.Format("SR.XmlInvalidSurrogate, {0}", ch.ToString("X", CultureInfo.InvariantCulture)), "ch"));
        ...
        public SurrogateChar(char lowChar, char highChar)
        {
            if (lowChar < surLowMin || lowChar > surLowMax)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(string.Format("SR.XmlInvalidLowSurrogate, {0}", ((int)lowChar).ToString("X", CultureInfo.InvariantCulture)), "lowChar"));

            if (highChar < surHighMin || highChar > surHighMax)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(string.Format("SR.XmlInvalidHighSurrogate, {0}", ((int)highChar).ToString("X", CultureInfo.InvariantCulture)), "highChar"));
