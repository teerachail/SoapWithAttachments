...
namespace TheS.Text
{
    ...
    class Base64Encoding : Encoding
    {
        ...
        public override int GetMaxByteCount(int charCount)
        {
            ...
            if ((charCount % 4) != 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(string.Format("SR.XmlInvalidBase64Length, {0}", charCount.ToString(NumberFormatInfo.CurrentInfo))));
        ...
        unsafe public override int GetByteCount(char[] chars, int index, int count)
        {
            ...
            if (index < 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("index", "SR.ValueMustBeNonNegative"));
            if (index > chars.Length)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("index", string.Format("SR.OffsetExceedsBufferSize, {0}", chars.Length)));
            if (count < 0)
            ...
            if (count > chars.Length - index)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("count", string.Format("SR.SizeExceedsRemainingBufferSpace, {0}", chars.Length - index)));

            if (count == 0)
                return 0;
            if ((count % 4) != 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(string.Format("SR.XmlInvalidBase64Length, {0}", count.ToString(NumberFormatInfo.CurrentInfo))));
                ...
                fixed (char* _chars = &chars[index])
                {
                    ...
                    while (pch < pchMax)
                    {
                        ...
                        if ((pch0 | pch1 | pch2 | pch3) >= 128)
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(string.Format("SR.XmlInvalidBase64Sequence, {0}, {1}", new string(pch, 0, 4), index + (int)(pch - _chars))));
                ...
                fixed (char* _chars = &chars[charIndex])
                {
                    fixed (byte* _bytes = &bytes[byteIndex])
                    {
                        ...
                        while (pch < pchMax)
                        {
                            ...
                            if ((pch0 | pch1 | pch2 | pch3) >= 128)
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(string.Format("SR.XmlInvalidBase64Sequence, {0}, {1}", new string(pch, 0, 4), charIndex + (int)(pch - _chars))));
                            // xx765432 xx107654 xx321076 xx543210
                            // 76543210 76543210 76543210
                ...
                fixed (byte* _chars = &chars[charIndex])
                {
                    fixed (byte* _bytes = &bytes[byteIndex])
                    {
                        ...
                        while (pch < pchMax)
                        {
                            ...
                            byte pch3 = pch[3];
                            if ((pch0 | pch1 | pch2 | pch3) >= 128)
                                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new FormatException(string.Format("SR.XmlInvalidBase64Sequence, {0} {1}", new string((sbyte*)pch, 0, 4), charIndex + (int)(pch - _chars))));
                            // xx765432 xx107654 xx321076 xx543210
                            // 76543210 76543210 76543210
        ...
        public override int GetMaxCharCount(int byteCount)
        {
            if (byteCount < 0 || byteCount > int.MaxValue / 4 * 3 - 2)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("byteCount", string.Format("SR.ValueMustBeInRange, {0}, {1}", 0, int.MaxValue / 4 * 3 - 2)));
