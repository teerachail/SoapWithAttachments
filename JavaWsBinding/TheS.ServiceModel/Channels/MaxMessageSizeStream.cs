...
namespace TheS.ServiceModel.Channels
{
    public class MaxMessageSizeStream : DelegatingStream
    {
        ...
        {
            string message = string.Format("SR.MaxReceivedMessageSizeExceeded, {0}", maxMessageSize);
        ...
        {
            string message = string.Format("SR.MaxSentMessageSizeExceeded, {0}", maxMessageSize);
            Exception inner = new QuotaExceededException(message);
