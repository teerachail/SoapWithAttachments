...
namespace TheS.Xml
{
    ...
    class XmlMtomWriter : XmlDictionaryWriter, IXmlMtomWriterInitializer
    {
        ...
        public void SetOutput(Stream stream, Encoding encoding, int maxSizeInBytes, string startInfo, string boundary, string startUri, bool writeMessageHeaders, bool ownsStream)
        {
            ...
            if (maxSizeInBytes < 0)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("maxSizeInBytes", "SR.ValueMustBeNonNegative"));
        ...
        void Initialize(Stream stream, string startInfo, string boundary, string startUri, bool writeMessageHeaders, bool ownsStream)
        {
            ...
            if (!MailBnfHelper.IsValidMimeBoundary(boundary))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentException(string.Format("SR.MtomBoundaryInvalid, {0}", boundary), "boundary"));
        ...
        void ThrowIfElementIsXOPInclude(string prefix, string localName, string ns)
        {
            ...
            if (localName == MtomGlobals.XopIncludeLocalName && ns == MtomGlobals.XopIncludeNamespace)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(string.Format("SR.MtomDataMustNotContainXopInclude, {0}, {1}", MtomGlobals.XopIncludeLocalName, MtomGlobals.XopIncludeNamespace)));
        }
        ...
        public override void WriteBase64(byte[] buffer, int index, int count)
        {
            if (Writer.WriteState == WriteState.Element)
            {
                ...
                if (count > buffer.Length - index)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new ArgumentOutOfRangeException("count", string.Format("SR.SizeExceedsRemainingBufferSpace, {0}", buffer.Length - index)));
        ...
        internal static int ValidateSizeOfMessage(int maxSize, int offset, int size)
        {
            if (size > (maxSize - offset))
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(string.Format("SR.MtomExceededMaxSizeInBytes, {0}", maxSize)));
            return size;
        }
