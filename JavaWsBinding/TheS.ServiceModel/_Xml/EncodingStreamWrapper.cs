    ...
    class EncodingStreamWrapper : Stream
    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (expectedEnc == SupportedEncoding.None)
                            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.XmlDeclarationRequired"));
                    }
                }
            }
            catch (DecoderFallbackException ex)
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.XmlInvalidBytes", ex));
            }
        }
        ...
        static Encoding GetEncoding(SupportedEncoding e)
        {
            switch (e)
            {
                ...
                default:
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.XmlEncodingNotSupported"));
            }
        }
        ...
        SupportedEncoding ReadBOMEncoding(bool notOutOfBand)
        {
            ...
            if (b4 == -1)
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.UnexpectedEndOfFile"));
        ...
        static SupportedEncoding ReadBOMEncoding(byte b1, byte b2, byte b3, byte b4, bool notOutOfBand, out int preserve)
        {
            ...
            else if (b1 == 0x00 && b2 == '<') // UTF-16 big endian, no BOM
            {
                ...
                if (notOutOfBand && (b3 != 0x00 || b4 != '?'))
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.XmlDeclMissing"));
                preserve = 4;
            }
            ...
            else if (b1 == 0xEF && b2 == 0xBB) // UTF8 with BOM
            {
                ...
                if (notOutOfBand && b3 != 0xBF)
                    throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.XmlBadBOM"));
                preserve = 1;
            }
            else  // Assume UTF8
            {
        ...
        static void CheckUTF8DeclarationEncoding(byte[] buffer, int offset, int count, SupportedEncoding e, SupportedEncoding expectedEnc)
        {
            ...
            if (encEq < 28) // Earliest second "=" can appear
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.XmlMalformedDecl"));
        ...
        internal static ArraySegment<byte> ProcessBuffer(byte[] buffer, int offset, int count, Encoding encoding)
        {
            ...
            {
                throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException("SR.XmlInvalidBytes", e));
            }
        }

        static void ThrowExpectedEncodingMismatch(SupportedEncoding expEnc, SupportedEncoding actualEnc)
        {
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(string.Format("SR.XmlExpectedEncoding, {0}, {1}", GetEncodingName(expEnc), GetEncodingName(actualEnc))));
        }
        ...
        {
            throw DiagnosticUtility.ExceptionUtility.ThrowHelperError(new XmlException(string.Format("SR.XmlEncodingMismatch, {0}, {1}", declEnc, docEnc)));
        }
